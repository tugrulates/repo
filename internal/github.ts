...
export async function getRemoteRepo(
  ...
  const remote = await gitRemoteUrl(options);
...
export async function createPullRequest(
  ...
  const repo = await getRemoteRepo(options);
  const head = options?.head ?? await gitCurrentBranch(options);
  if (!head) throw new Error("Cannot determine PR branch in detached HEAD");
  const base = options?.base ?? await gitRemoteBase(options);
  if (!base) throw new Error("Cannot determine remote base branch");
  const commit = !options?.title
    ? (await gitLog({ from: base })).pop()
    : undefined;
  const title = options?.title ?? commit?.title;
